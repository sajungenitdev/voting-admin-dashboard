          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-white/40 text-sm mt-0.5">Manage all active and expired subscriptions</p>
              </div>
            </div>
          </div>
          <button
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-red-500/10">
            <thead className="bg-red-500/5">
              <tr>
                <th className="px-6 py-3 text-xs font-medium text-left text-red-400 uppercase">Company</th>
                <th className="px-6 py-3 text-xs font-medium text-left text-red-400 uppercase">Plan</th>
                <th className="px-6 py-3 text-xs font-medium text-left text-red-400 uppercase">Amount</th>
                <th className="px-6 py-3 text-xs font-medium text-left text-red-400 uppercase">Start Date</th>
                <th className="px-6 py-3 text-xs font-medium text-left text-red-400 uppercase">End Date</th>
                <th className="px-6 py-3 text-xs font-medium text-left text-red-400 uppercase">Status</th>
                <th className="px-6 py-3 text-xs font-medium text-left text-red-400 uppercase">Auto Renew</th>
              </tr>
            </thead>
              ...
              {subscriptions.length === 0 ? (
                ...
                subscriptions.map((sub, idx) => (
                  <tr key={idx} className="transition-colors hover:bg-red-500/5">
                    <td className="px-6 py-4 text-sm text-white/80">{sub.user?.companyName || "N/A"}</td>
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getPlanColor(sub.tier)}`}>
                        {sub.tier?.toUpperCase()}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-white/80">${sub.price} / ৳{sub.priceBDT}</td>
                    <td className="px-6 py-4 text-sm text-white/40">{new Date(sub.startDate).toLocaleDateString()}</td>
                    <td className="px-6 py-4 text-sm text-white/40">{new Date(sub.endDate).toLocaleDateString()}</td>
                    <td className="px-6 py-4">
